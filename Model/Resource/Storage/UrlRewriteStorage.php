...
class UrlRewriteStorage
{
    ...
    protected function getExistingUrlRewrites(array $productIds, array $allStoreIds)
    {
        ...
        foreach ($collection as $storeId => $ids) {
            ...
            foreach ($urlRewriteData as $datum) {
                ...
                $data[$storeId][$urlRewrite->getProductId()][(int)$urlRewrite->getCategoryId()][$urlRewrite->getKey()] = $urlRewrite;
            }
        }
    ...
    protected function getExistingVisibilities(array $productIds)
    {
        ...
        $visibilities = [];
        foreach ($rows as $row) {
            $visibilities[$row['entity_id']][$row['store_id']] = (int)$row['value'];
        }
    ...
    /**
     * Updates all url_rewrites from a single product / store view / category_id (may be null)
     */
    ...
    {
        $oldRewrites = array_key_exists((int)$categoryId, $existingUrlRewrites) ? $existingUrlRewrites[(int)$categoryId] : [];
        $newRewrites = $this->collectNewRewrites($productId, $storeViewId, $existingUrlRewrites, $categoryId, $allProductUrlKeys);
    ...
    protected function collectNewRewrites(int $productId, int $storeViewId, array $existingUrlRewrites, $categoryId, array $allProductUrlKeys)
    {
        ...
        if (isset($existingUrlRewrites[(int)$categoryId])) {
            /** @var UrlRewrite $urlRewrite */
            foreach ($existingUrlRewrites[(int)$categoryId] as $urlRewrite) {
                if ($urlRewrite->getRedirectType() === self::NO_REDIRECT) {
                    // if there is more than one zero rewrites (should not occur), just pick one
                    $existingZeroRewrite = $urlRewrite;
                } else {
                    // if not (any more) rewriting, update existing rewrites that have the old request path as a target
        ...
        if ($this->metaData->saveRewritesHistory) {
            if ($existingZeroRewrite) {

                if (!$existingZeroRewrite->equals($currentZeroRewrite)) {
     ...
     * @return string|null
     */
    protected function createRequestPath(int $productId, int $storeViewId, $categoryId, array $allProductUrlKeys)
    {
        $pieces = [];
    ...
    protected function replaceIntoUrlRewrite(UrlRewrite $urlRewrite)
    {
        ...
        if ($urlRewrite->getCategoryId() !== null && $urlRewrite->hasExtension()) {
